...
def register_admin_handlers(bot):
    ...
    # -------------------------------------------------
    # HELPER: Stop Loading & Handle Errors
    # -------------------------------------------------
    def safe_run(call, func):
        try:
            bot.answer_callback_query(call.id) # 🛑 STOP LOADING INSTANTLY
            func()
        except Exception as e:
            print(f"❌ Admin Panel Error: {e}")
            traceback.print_exc()
            # এরর হলে ইউজারকে পপআপ দেখানো হবে
    ...
    def export_users(call):
        def action():
            users = load_users()
            if not users:
                bot.answer_callback_query(call.id, "❌ No users found yet.", show_alert=True)
                return
        ...
        def action():
            if os.path.exists(CUSTOM_FILE):
                with open(CUSTOM_FILE, 'rb') as f:
                    bot.send_document(call.message.chat.id, f, caption="✅ <b>Settings Backup</b>", visible_file_name="custom_data.json")
            else:
